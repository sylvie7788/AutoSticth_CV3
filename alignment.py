...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        samples = random.sample(matches, s)

        homo = np.eye(3, 3)

        if m == eTranslate:
            n = matches[0]
            (ax, ay) = f1[n.queryIdx].pt
            (bx, by) = f2[n.trainIdx].pt

            homo[0, 2] = bx - ax
            homo[1, 2] = by - ay
        else:
            homo = computeHomography(f1, f2, samples)
        
        inlier_indices = getInliers(f1, f2, matches, homo, RANSACthresh)

        if len(inlier_indices) > len(largest_idx):
            largest_idx = inlier_indices

    M = leastSquaresFit(f1, f2, matches, m, largest_idx)
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        p = np.array([ax, ay, 1])
        p_predict = np.dot(M, p) 

        p_predict[0] = p_predict[0] / p_predict[2]
        p_predict[1] = p_predict[1] / p_predict[2]

        dist = np.sqrt((bx - p_predict[0])**2 + (by - p_predict[1])**2)
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for i, inlier_index in enumerate(inlier_indices):
            inliers.append(matches[inlier_index])
